...
--2.	Listar o número do pedido, data de entrega, a quantidade e a descrição do produto com código 115.
...
WHERE EXISTS (
    ...
    AND EXISTS (
        SELECT 1
        FROM cliente c
        WHERE p.cod_cliente = c.Cod_cliente AND c.Nome_cliente = 'Ciclano da Silva'
    )
);
...
--para todos os clientes que moram em Sorocaba.
